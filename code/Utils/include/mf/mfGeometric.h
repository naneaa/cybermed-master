#ifndef MFGEOMETRIC_H
#define MFGEOMETRIC_H
...
#define SPACE typename mfGeometric<_Traits>::space
//#define SMESH typename mfGeometric<_Traits>::sMesh

template <class _Traits> class mfGeometric
{
public:
	 ...
	 */
	mfGeometric(sMesh *_mesh = NULL, double _erro = MF_ERRO);
...
template <class _Traits> mfGeometric<_Traits>::mfGeometric(sMesh *_mesh, double _erro)
{
	mesh = _mesh;
	temp = 0;
	erro = _erro;
...
template <class _Traits> mfGeometric<_Traits>::~mfGeometric()
{
}

template <class _Traits> void mfGeometric<_Traits>::setMesh(sMesh *_mesh)
{
	mesh = _mesh;
}
...
template <class _Traits> SPACE mfGeometric<_Traits>::dist(sVertex *p0, sVertex *p1 MF_DMUTEXV)
{
...
template <class _Traits> SPACE mfGeometric<_Traits>::dist(space *p0, space *p1 MF_DMUTEXV)
{
...
template <class _Traits> int mfGeometric<_Traits>::orientation3D(ids *idvertices, sVertex **vertices MF_DMUTEXV)
{
	...
	x0 = vertices[1]->getCoord(0 MF_MUTEXV) - vertices[0]->getCoord(0 MF_MUTEXV);
	y0 = vertices[1]->getCoord(1 MF_MUTEXV) - vertices[0]->getCoord(1 MF_MUTEXV);
	z0 = vertices[1]->getCoord(2 MF_MUTEXV) - vertices[0]->getCoord(2 MF_MUTEXV);

	x1 = vertices[2]->getCoord(0 MF_MUTEXV) - vertices[0]->getCoord(0 MF_MUTEXV);
	y1 = vertices[2]->getCoord(1 MF_MUTEXV) - vertices[0]->getCoord(1 MF_MUTEXV);
	z1 = vertices[2]->getCoord(2 MF_MUTEXV) - vertices[0]->getCoord(2 MF_MUTEXV);

	x2 = vertices[3]->getCoord(0 MF_MUTEXV) - vertices[0]->getCoord(0 MF_MUTEXV);
	y2 = vertices[3]->getCoord(1 MF_MUTEXV) - vertices[0]->getCoord(1 MF_MUTEXV);
	z2 = vertices[3]->getCoord(2 MF_MUTEXV) - vertices[0]->getCoord(2 MF_MUTEXV);
	
	det = x0*y1*z2 + y0*z1*x2 + z0*x1*y2 - z0*y1*x2 - y0*x1*z2 - x0*z1*y2;
