...
template <class sTraits> class mfPgReader : public mfTReader<sTraits>
{
public:
	 ...
	 * \param
	 */
	mfPgReader(int threads_vertices = 2, int threads_cells = 2);
...
private:
	...
	typedef typename mfTReader<sTraits>::mfTReaderData mfTReaderData;
...
template <class sTraits> mfPgReader<sTraits>::mfPgReader(int threads_vertices, int threads_cells)
	:mfTReader<sTraits>(threads_vertices, threads_cells)
{
}

template <class sTraits> mfPgReader<sTraits>::~mfPgReader()
{
}

template <class sTraits> bool mfPgReader<sTraits>::read(sMesh* malha, char* xml)
{
	MF_ASSERT(malha);
	MF_ASSERT(xml);
	...
	mfTReaderData *pdobj = new mfTReaderData[this->nthreadsv]();
	...
	pdobj = new mfTReaderData[this->nthreadsc]();
	MF_ASSERT(tid);
	MF_ASSERT(pdobj);
...
template <class sTraits> void* mfPgReader<sTraits>::threadPoint(mfTReaderData* obj)
{
	MF_ASSERT(obj);
   	PGconn     *conn;
   	PGresult   *res;
	...
	while(start <= obj->end)
	{
		...
		else
		{
			...
			for(i = 0; i < num; i++)
			{
				...
				coords[0]  = atmf(PQgetvalue(res, i, x_fnum));			
				coords[1]  = atmf(PQgetvalue(res, i, y_fnum));
				if(sVertex::getDimension() == 3)
					coords[2]  = atmf(PQgetvalue(res, i, z_fnum));
...
template <class sTraits> void* mfPgReader<sTraits>::threadCell(mfTReaderData* obj)
{
	MF_ASSERT(obj);
   	PGconn     *conn;
   	PGresult   *res;
