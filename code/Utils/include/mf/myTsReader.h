#ifndef MYTSREADER_H
#define MYTSREADER_H

#include <fstream>

#include "mfMacros.h"
#include "mfMesh.h"
#include "mfReader.h"

using namespace std;

namespace mf
{

template <class _Traits> class myTsReader : public mfReader<_Traits>
{
public:
	typedef typename _Traits::space space;		/**< Space typename definition */
	typedef typename _Traits::ids ids;		/**< Id typename definition */
	typedef typename _Traits::sVertex sVertex;	/**< Vertex typename definition */
	typedef typename _Traits::sCell sCell;		/**< Cell typename definition */
	typedef typename _Traits::sMesh sMesh;		/**< Mesh typename definition */

	/** Constructor */
	myTsReader();

	/** Destructor */
	~myTsReader();

	virtual bool read(sMesh* malha, const char* filename);

private:

	bool reader(ifstream &pf, sMesh* malha);
};

template <class _Traits> myTsReader<_Traits>::myTsReader()
{
}

template <class _Traits> myTsReader<_Traits>::~myTsReader()
{
}

template <class _Traits> bool myTsReader<_Traits>::read(sMesh* malha, const char *filename)
{
	MF_ASSERT(filename);
	MF_ASSERT(malha);

	ifstream pf(filename);

	if( pf.is_open())
	{
		if(! reader(pf, malha))
			return false;

		pf.close();
	}
	else
		return false;

	return true;
}

template <class _Traits> bool myTsReader<_Traits>::reader(ifstream &pf, sMesh* malha)
{
	char auxstr[255];
	space coords[3];
	sCell cell;
	int i, nv, j, nc, k, idnv;
	ids auxvtx[4];
	sVertex *v;
	int vId;
	float value;
	float max = -1000, min = 1000;

	MF_ASSERT(auxstr);

	//while( (! pf.eof())&&(strcmp(auxstr,"OFF") != 0) )
	//	pf >> auxstr;

	//if(! pf.eof())
	{
		pf >> nv;
		pf >> nc;
		pf >> k;

		MF_ASSERT(nv > 0);
	
		for (i=0; i<nv; i++)
		{
			for(j = 0; j < 3; j++)
				pf >> coords[j];

			vId = malha->addVertex(coords);

			pf >> value;

			/*if(max < value)
				max = value;
			if(min > value)
				min = value;*/

			//malha->getVertex(vId)->setValue(value);
		}

		MF_ASSERT(nc >= 0);

		//cout << "Vertices Lidos" << endl;

		for (i=0; i<nc; i++)
		{
			//pf >> idnv;

			//MF_ASSERT(idnv <= cell.getNumberVerticesInCell());

			for (j=0; j < 4 ; j++)
			{
				pf >> k;
				MF_ASSERT((k >= 0)&&(k < nv));
				auxvtx[j] = k;
			}
			/*for(j=idnv; j < 4; j++)
			{
				auxvtx[j] = -1;
			}*/

			//cerr << "c " << i << " " << auxvtx[0] << " " << auxvtx[1] << " " << auxvtx[2] << " " << auxvtx[3] << endl;

			malha->addCell(auxvtx);
		}
		//cerr << "Celulas Lidos" << endl;
	}
	//else
	//	return false;

	//cout << "Min " << min << " Max " << max << endl;

	return true;
}

}

#endif
