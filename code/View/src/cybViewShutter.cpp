...
#include "cybViewShutter.h"

	CybViewShutter::CybViewShutter(CybMouse* mouse) : Cyb3DWorld(mouse, true)
	{
		Cyb3DWorld : viewNumber = 4; //sets the visualization method
	}
	
	void CybViewShutter::display()
	{
		...
		if(cybCore->getNumInterators() > 0)
		{
			...
			for (int i = 0; i < numInterators; i++) {
				...
				if (interator->isEnabled()) {
					...
					//index = cybCore->activeInterator;
					mfCellsIterator<cybTraits> cellsIterator(interatorMesh);
