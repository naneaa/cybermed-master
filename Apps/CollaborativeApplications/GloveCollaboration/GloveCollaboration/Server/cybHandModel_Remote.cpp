...
CybHandModel_Remote::CybHandModel_Remote() {
...
void CybHandModel_Remote::draw() {
	...
	glPushMatrix();
		...
		glMultMatrixf(position); //set the position
...
/*This function is used for the Collaboration to set the hand's position and the finger's position*/
void CybHandModel_Remote::setMatrix(float ar[]) {
	for (int i = 0; i < 18; ++i) {
		eixo[i] = ar[i];
	}
	int k = 0;
...
/*This function is used for the Collaboration to get the values such as hand's position and the finger's position to send to others users
  This Class is used for  user that receive the matrix, thus, implement this function is optional.
  If this model was used for the user with the glove, this function must be implemented*/
float *CybHandModel_Remote::getMatrix() {
	
}

/*This function is necessary only for the glove's user*/
void CybHandModel_Remote::setUpdateModel(string type) { 
	//empty
}
